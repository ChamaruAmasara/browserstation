...
SOCKET_URL = API_URL.replace("http", "ws")
...
async def browse_wikipedia(page, browser_num, max_time=40):
    ...
    print(
        f"[Browser {browser_num}] Navigating to Wikipedia: {start_url.split('/')[-1]}..."
    )
    try:
    ...
    if browser_num == 1:
        try:
            ...
            if await featured_article.count() > 0:
                ...
                print(
                    f"[Browser {browser_num}] Clicking on featured article: {article_text[:50] if article_text else 'Unknown'}..."
                )
    ...
    try:
        ...
        if browser_num % 3 == 0:
            ...
            article_links = await page.locator(
                "#mw-content-text li a[href*='/wiki/']"
            ).all()
        else:
            # Look for any content links
            article_links = (
                await page.locator("#content a[href*='/wiki/']")
    ...
    try:
        ...
        print(
            f"[Browser {browser_num}] Visiting Wikipedia portal: {portal_url.split(':')[-1]}"
        )
        ...
        if visible_portal_links:
            ...
            if browser_num % 2 == 0:
                selected_link = visible_portal_links[
                    min(browser_num * 2, len(visible_portal_links) - 1)
                ]
            else:
        ...
        if stories:
            ...
            for i, story_index in enumerate(story_indices[:1]):  # First story
                if story_index < len(stories):
                    ...
                    print(
                        f"[Browser {browser_num}] Clicking on story #{story_index + 1}: {story_title}"
                    )
                    ...
                    if href and href.startswith("http"):
                        ...
                        try:
                            ...
                            print(
                                f"[Browser {browser_num}] Browsed external article: {story_title}"
                            )
                        except Exception as e:
                            print(
                                f"[Browser {browser_num}] Failed to load external article: {e}"
                            )
                        finally:
    ...
    try:
        ...
        if comment_links:
            ...
            if comment_index < len(comment_links):
                ...
                print(
                    f"[Browser {browser_num}] Reading comments on story #{comment_index + 1}: {comments_text}"
                )
    ...
    if stories:
        ...
        if final_index < len(stories):
            ...
            try:
                ...
                print(
                    f"[Browser {browser_num}] Final story #{final_index + 1}: {story_title}"
                )
...
async def browse_session(browser_num, start_delay):
    ...
    try:
        ...
        resp = requests.post(f"{API_URL}/browsers", timeout=30, headers=headers)
        ...
        for retry in range(max_retries):
            try:
                details = requests.get(
                    f"{API_URL}/browsers/{browser_id}", timeout=30, headers=headers
                ).json()
                if "websocket_url" in details:
                    ...
                    print(
                        f"[Browser {browser_num}] Waiting for websocket URL... (attempt {retry + 1})"
                    )
                    await asyncio.sleep(2)
            except Exception as e:
        ...
        if websocket_url.startswith("ws"):
            ...
            ws_url = f"{SOCKET_URL}{websocket_url}"
        ...
        try:
            ...
            if wiki_elapsed < 40:
                ...
                print(
                    f"[Browser {browser_num}] Waiting {wait_time:.1f}s to complete Wikipedia section..."
                )
            ...
            if hn_elapsed < 40:
                ...
                print(
                    f"[Browser {browser_num}] Waiting {wait_time:.1f}s to complete HN section..."
                )
                await asyncio.sleep(wait_time)
        ...
        print(
            f"[Browser {browser_num}] Total session time: {total_elapsed:.1f} seconds (browsing: 90.0s)"
        )
        ...
        if browser_id:
            try:
                ...
                requests.delete(
                    f"{API_URL}/browsers/{browser_id}", timeout=10, headers=headers
                )
            except Exception as e:
...
async def main():
    ...
    print(
        f"Starting {NUM_BROWSERS} browser sessions with {STAGGER_DELAY} second delays..."
    )
    print(
        "Each browser will visit different Wikipedia topics and Hacker News stories.\n"
    )
